module TagBuddy
  ...
  module Base
    ...
    def buddy_tag(*args)
      ...
      is_new_tag_on_item_for_user = ($redis.sadd data[:user].storage_key_for_tag_items(data[:tag].buddy_named_scope), data[:item].buddy_named_scope)
      ...
      tag_array.push(data[:tag].buddy_named_scope).uniq!
      $redis.hset data[:user].storage_key(:items, :tags), data[:item].buddy_named_scope, ActiveSupport::JSON.encode(tag_array)
    ...
    def buddy_untag(*args)
      ...
      was_removed_tag_on_item_for_user = ($redis.srem data[:user].storage_key_for_tag_items(data[:tag].buddy_named_scope), data[:item].buddy_named_scope)
      ...
      # (this is kept in a dictionary to save memory)
      tag_array = data[:user].tags_via(data[:item])
      tag_array.delete(data[:tag].buddy_named_scope)
      $redis.hset data[:user].storage_key(:items, :tags), data[:item].buddy_named_scope, ActiveSupport::JSON.encode(tag_array)
    end
